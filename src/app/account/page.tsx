...
export default function AccountPage() {
  ...
  const displayName = me?.displayName || t('accountPageDefaultName');
  ...
  return (
    <div className="min-h-screen bg-bg py-10 px-4">
      <div className="max-w-2xl mx-auto space-y-8">
        <div>
          <h1 className="text-2xl font-bold text-ink">{t('accountPageHeading')}</h1>
          <p className="text-ink3 text-sm mt-1">{t('accountPageSubtitle')}</p>
        </div>
        ...
        <div className="bg-panel border border-line rounded-2xl p-6">
          <p className="text-xs font-semibold text-ink3 uppercase tracking-wide mb-1">
            {t('accountPageContinueInApp')}
          </p>
          <p className="text-sm text-ink2 mb-4">
            {t('accountPageContinueInAppBody')}
          </p>
          <div className="flex gap-3 flex-wrap">
            <StoreBadge href={IOS_STORE} label={t('accountPageIosLabel')} store="App Store" icon="🍎" />
            <StoreBadge href={ANDROID_STORE} label={t('accountPageAndroidLabel')} store="Google Play" icon="▶" />
          </div>
        </div>
      </div>
    </div>
    ...
    return (
      <span className="inline-flex items-center gap-3 px-4 py-2.5 rounded-xl bg-panel2 text-ink3 opacity-70 cursor-default">
        ...
        <span className="leading-tight">
          ...
          <span className="block text-sm font-bold">{tp.accountPageStoreSoon(store)}</span>
        </span>
      </span>
    );
  }
