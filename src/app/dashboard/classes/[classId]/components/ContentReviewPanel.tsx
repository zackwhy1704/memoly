...
/**
 * Shared review panel used by both the ReviewTab and the wizard step 4.
 * `onAllReviewed` fires once when draft item count hits 0 (either everything
 * approved/rejected or the list arrives empty after compile).
 */
export function ContentReviewPanel({
  ...
  // Fire onAllReviewed once when drafts reach 0 after data loads.
  const notifiedRef = useRef(false);
  useEffect(() => {
    if (!query.isLoading && !query.isError && draftItems.length === 0 && !notifiedRef.current) {
