...
export function ReportTab({ orgId, classId }: { orgId: string; classId: string }) {
  ...
  if (query.isLoading || report?.status === 'generating') {
    return (
      <div className="flex flex-col items-center gap-3 py-16 text-ink3">
        ...
        <p className="text-sm">{t('reportTabGenerating')}</p>
      </div>
    );
  }
  ...
  if (report?.status === 'failed') {
    ...
    return (
      <ErrorView
        message={report.message ?? t('reportTabGenerationFailed')}
  ...
  function handlePrint() {
    ...
    win.document.write(`
      ...
      <title>${t('reportTabPrintTitle')}</title>
      ...
      <h1>${t('reportTabPrintHeading')}</h1>
      <small>${t('reportTabPrintGenerated')} ${generatedDate}${cached ? ' (cached)' : ''}</small>
      ${narrative.split('\n\n').map((p) => `<p>${p}</p>`).join('')}
      </body></html>`);
    win.document.close();
    win.print();
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h2 className="text-base font-semibold text-ink">{t('reportTabHeading')}</h2>
          <p className="text-xs text-ink3 mt-0.5">
            {tp.reportTabGenerated(generatedDate, cached)}
          </p>
        </div>
