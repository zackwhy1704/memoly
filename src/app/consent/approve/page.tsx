    ...
    <>
      {view.state === 'loading' && (
        <div className="mt-8 flex flex-col items-center gap-4">
          ...
          <p className="text-[#6B604E] text-sm">Processing consent…</p>
        </div>
      )}

      {view.state === 'success' && (
        <div className="mt-6 bg-white border border-[#E8E1D4] rounded-2xl p-8 shadow-sm">
          <div className="text-4xl mb-4">✅</div>
          <h2 className="text-lg font-bold text-[#2A2118] mb-2">Consent granted!</h2>
          <p className="text-[#6B604E] text-sm">
      ...
      {view.state === 'already_used' && (
        <div className="mt-6 bg-white border border-[#E8E1D4] rounded-2xl p-8 shadow-sm">
          ...
          <h2 className="text-lg font-bold text-[#2A2118] mb-2">Already approved</h2>
          <p className="text-[#6B604E] text-sm">
      ...
      {view.state === 'expired' && (
        <div className="mt-6 bg-white border border-[#E8E1D4] rounded-2xl p-8 shadow-sm">
          <div className="text-4xl mb-4">⏰</div>
          <h2 className="text-lg font-bold text-[#2A2118] mb-2">Link expired</h2>
          <p className="text-[#6B604E] text-sm">
      ...
      {view.state === 'invalid' && (
        <div className="mt-6 bg-white border border-[#E8E1D4] rounded-2xl p-8 shadow-sm">
          <div className="text-4xl mb-4">🔍</div>
          <h2 className="text-lg font-bold text-[#2A2118] mb-2">Link not found</h2>
          <p className="text-[#6B604E] text-sm">
      ...
      {view.state === 'error' && (
        <div className="mt-6 bg-white border border-[#E8E1D4] rounded-2xl p-8 shadow-sm">
          <div className="text-4xl mb-4">⚠️</div>
          <h2 className="text-lg font-bold text-[#2A2118] mb-2">Something went wrong</h2>
          <p className="text-[#6B604E] text-sm">{view.message}</p>
          <button
            onClick={retry}
            className="mt-4 px-6 py-2 bg-accent text-white text-sm font-bold rounded-lg hover:bg-accent/80 transition"
          >
            Try again
          </button>
        </div>
  ...
  return (
    <main className="min-h-screen bg-[#FAFAFF] flex items-center justify-center px-4">
      <div className="w-full max-w-sm text-center">
        ...
        <h1 className="text-2xl font-extrabold text-[#2A2118] mb-2">Apalchi</h1>
        <Suspense
          fallback={
            <div className="mt-8 flex flex-col items-center gap-4">
              <span className="w-8 h-8 border-4 border-accent border-t-transparent rounded-full animate-spin" />
              <p className="text-[#6B604E] text-sm">Loading…</p>
            </div>
          }
        >
          <ConsentApproveBody />
